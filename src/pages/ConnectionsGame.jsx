...
function ConnectionsGame() {
  ...
  useEffect(() => {
    const hasSeenHowToPlay = localStorage.getItem('connections-wedding-how-to-play');
    if (!hasSeenHowToPlay) {
      setShowHowToPlay(true);
      localStorage.setItem('connections-wedding-how-to-play', 'true');
    }
  }, []);
  ...
  useEffect(() => {
    ...
    if (stateToLoad) {
      ...
      if (state.solved && state.solved.length === PUZZLE.categories.length) {
        ...
        // Also handle game over state from load
        setViewOnlyMode(true);
        setIsGameOver(true);
        setShowCompletionModal(false);
      }
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 py-4 sm:py-8 px-3 sm:px-4">
      ...
      <CompletionModal
        ...
        icon={mistakes < 4 ? "🎉" : "😔"}
        title={mistakes < 4 ? "Congratulations!" : "Unlucky!"}
        message={
          mistakes < 4
            ? mistakes === 0
              ? "Perfect!"
        ...
        stats={[
          {
            label: 'Score',
            value: `${solved.filter(s => !s.isRevealed).length}/4`,
            gradient: mistakes < 4
              ? 'from-yellow-100 to-green-100 dark:from-yellow-900/30 dark:to-green-900/30'
      ...
      <div className="w-full max-w-[min(96vw,650px)] mx-auto">
        ...
        <div className="space-y-2 mb-6">
          {solved.sort((a, b) => a.difficulty - b.difficulty).map((category, idx) => (
            ...
            >
              <h3 className="font-semibold text-center mb-2 uppercase text-gray-900">{category.name}</h3>
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none space-y-2">
                  ...
                  <div className="flex gap-1 sm:gap-2">
                    {solving.words.map(word => (
                      <div key={word} className="flex items-center justify-center aspect-square px-2 py-1 font-semibold uppercase text-center rounded-md text-gray-900" style={{ fontSize: 'clamp(0.7rem, 2.5vw, 1rem)' }}>
                        {word}
                      </div>
                    ))}
                  </div>
